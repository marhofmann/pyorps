...
class TestRasterHandler(unittest.TestCase):
    ...
    def test_coords_to_indices(self):
        ...
        with patch('rasterio.transform.rowcol') as mock_rowcol:
            ...
            # Should adjust for window offset (5-3, 7-2) and (6-3, 8-2)
            expected = np.array([(2, 5), (3, 6)])
            np.testing.assert_array_equal(indices, expected)

            # Test with single coordinate tuple
            mock_rowcol.return_value = ([5], [7])  # Single row, col but as lists
            coord = (500050, 5599950)
            indices = handler.coords_to_indices(coord)
    ...
    def test_indices_to_coords(self):
        ...
        with patch('rasterio.transform.xy') as mock_xy:
            ...
            # Expected values would include adjustment for pixel center and offset
            # But since we're mocking xy, we just check that the right values come through
            expected = np.array([(500040, 5599960), (500060, 5599940)])
            np.testing.assert_array_almost_equal(coords, expected)

    def test_get_data_at_coords(self):
        """Test the get_data_at_coords method."""
        # Create a handler with known data values
        raster_data = np.zeros((1, 10, 10), dtype=np.uint16)
        raster_data[0, 5, 5] = 42  # Set a specific value at row=5, col=5
        ...
        # Mock coords_to_indices to return specific indices
        with patch.object(handler, 'coords_to_indices') as mock_to_indices:
            mock_to_indices.return_value = np.array([(5, 5)])  # Row=5, Col=5

            # Test get_data_at_coords
            coords = [(500050, 5599950)]
            values = handler.get_data_at_coords(coords)
    ...
    def test_apply_geometry_mask(self):
        ...
        with patch('rasterio.features.rasterize') as mock_rasterize:
            ...
            mask = np.zeros((handler.window.height, handler.window.width), dtype=np.uint8)
            ...
            # Check that values outside the mask are set to max value
            max_val = np.iinfo(handler.data.dtype).max
            for b in range(handler.data.shape[0]):
                # Left half (inside mask) should be unchanged (all 1s from our test data)
                np.testing.assert_array_equal(result[b, :, :mask.shape[1] // 2],
                                              np.ones_like(result[b, :, :mask.shape[1] // 2]))

                # Right half (outside mask) should be set to max value
                np.testing.assert_array_equal(result[b, :, mask.shape[1] // 2:],
                                              np.full_like(result[b, :, mask.shape[1] // 2:], max_val))

    def test_apply_line_buffer_mask(self):
        """Test the apply_line_buffer_mask method."""
        # Create a handler
        handler = RasterHandler(
            self.raster_dataset,
            self.source_coords,
        ...
        # Mock apply_geometry_mask since that's already tested
        with patch.object(handler, 'apply_geometry_mask') as mock_apply_mask:
            mock_apply_mask.return_value = handler.data  # Return unchanged data

            # Apply line buffer mask
            ...
            geom_arg = mock_apply_mask.call_args[0][0]
            self.assertIsInstance(geom_arg, Polygon)  # Should be buffered to a polygon
    ...
    def test_estimate_optimal_buffer_width(self):
        ...
        buffer_width = handler.estimate_optimal_buffer_width(
            (500050, 5599950),
            (500250, 5599950)
        )
    ...
    def test_apply_geometry_mask_with_specified_bands(self):
        ...
        with patch('rasterio.features.rasterize') as mock_rasterize:
            # Create a mask with 1s in the left half, 0s in the right
            mask = np.ones((handler.window.height, handler.window.width), dtype=np.uint8)
            mask[:, mask.shape[1] // 2:] = 0  # Right half is masked out
            mock_rasterize.return_value = mask

            # Apply the mask only to bands 0 and 2 (already 0-indexed for test)
            # The method will convert 0, 2 to -1, 1 which isn't what we want
            # So we provide 1, 3 as input which will become 0, 2 after conversion
            bands = [1, 3]  # will be converted to 0-indexed inside method
            result = handler.apply_geometry_mask(polygon, outside_value=999, bands=bands)

            # Check band 1 - index 0 (should be masked)
            ...
            # Check band 2 - index 1 (should be unmodified)
            np.testing.assert_array_equal(
                result[1],
                np.full_like(result[1], 20)  # All values should still be 20
            )

            # Check band 3 - index 2 (should be masked)
            expected_band2 = np.full_like(result[2], 30)  # Start with all 30s
            expected_band2[:, mask.shape[1] // 2:] = 999  # Right half should be 999
            np.testing.assert_array_equal(result[2], expected_band2)

    def test_get_data_at_coords_bounds_check(self):
        """Test get_data_at_coords with coordinates outside bounds."""
        # Create a handler
        handler = RasterHandler(
            self.raster_dataset,
            self.source_coords,
        ...
        # Mock coords_to_indices to return some valid and some invalid indices
        with patch.object(handler, 'coords_to_indices') as mock_indices:
            # Return one valid and one invalid index
            mock_indices.return_value = np.array([
                (2, 3),  # Valid index
                (100, 100)  # Invalid index (outside bounds)
            ])

            # Test with both coordinates
            coords = [(500050, 5599950), (500150, 5599850)]
            values = handler.get_data_at_coords(coords)

            # Should return one valid value and one None
            self.assertEqual(len(values), 2)
            self.assertEqual(values[0], 1)  # From our ones array
            self.assertIsNone(values[1])  # Out of bounds returns None
