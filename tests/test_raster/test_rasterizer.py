...
class TestGeoRasterizer(unittest.TestCase):
    ...
    def test_modify_raster_values(self):
        """Test modifying raster values."""
        rasterizer = GeoRasterizer(
            self.raster_dataset,
            self.cost_assumptions
        )
        ...
        replace_values = {1: 5}
        modified_raster = rasterizer.modify_raster_values(replace_values)

        # Check all values are now 5
        self.assertTrue(np.all(modified_raster == 5))
    ...
    def test_add_dataset(self):
        ...
        # Mock initialize_geo_dataset to test add_dataset
        with patch('pyorps.raster.rasterizer.initialize_geo_dataset') as mock_init:
            mock_dataset = MagicMock()
            mock_init.return_value = mock_dataset

            # Test add_dataset
            result = rasterizer.add_dataset('test_file.shp')

            # Check that initialize_geo_dataset was called correctly
            mock_init.assert_called_once()
            args, kwargs = mock_init.call_args
            self.assertEqual(args[0], 'test_file.shp')
            # Check the second positional argument instead of kwargs
            self.assertEqual(args[1], "EPSG:32632")

            # Check that the dataset was added to additional_datasets
            self.assertIn(mock_dataset, rasterizer.additional_datasets)
    ...
    def test_add_datasets(self):
        ...
        # Mock add_dataset method
        with patch.object(rasterizer, 'add_dataset') as mock_add:
            # Configure mock to return a new MagicMock for each call
            mock_add.side_effect = lambda dataset, **kwargs: dataset

            # Test add_datasets with multiple items
            datasets = ['file1.shp', 'file2.shp']
            rasterizer.add_datasets(datasets, crs="EPSG:4326")

            # Check that add_dataset was called for each dataset
            self.assertEqual(mock_add.call_count, 2)
            mock_add.assert_any_call('file1.shp', crs="EPSG:4326")
            mock_add.assert_any_call('file2.shp', crs="EPSG:4326")
