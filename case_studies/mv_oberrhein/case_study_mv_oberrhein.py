...
This Python file implements a comprehensive case study demonstrating how PYORPS can be integrated into distribution grid
planning for multi-criteria optimization. The study focuses on connecting a photovoltaic (PV) power plant to the
medium-voltage (MV) grid MV-Oberrhein using underground cables, considering technical constraints alongside economic and
environmental aspects.
The code leverages publicly available land registry data to create cost rasters that represent different terrain
types and their associated construction costs. It then uses PYORPS for pathfinding with varying neighborhood
structures (R0, R1, R2, R3) to find optimal cable routes between the PV plant and potential connection points. After
identifying possible routes, the code evaluates their technical feasibility through power flow simulations, checking
voltage constraints and equipment loading. The final result identifies the most cost-effective and technically valid
connection options for each neighborhood structure.
...
def get_bus_geoseries(net: pp.pandapowerNet, net_crs: str, buses: Optional[list[int]] = None) -> gpd.GeoSeries:
    """
...
def create_bbox_from_net_data(
    ...
    Args:
        ...
        create_gdf: whether to return a GeoDataFrame (True) or bounding coordinates (False)
        crs: coordinate reference system

    Returns:
        Either tuple of (minx, miny, maxx, maxy) or GeoDataFrame with the bounding box polygon
    """
...
def prepare_raster_data(
    ...
    Args:
        ...
        gis_data_path: path to the GIS data file
        save_path: path to save the raster data

    Returns:
        tuple of (raster, transform)
    """
    bbox = create_bbox_from_net_data(net, create_gdf=True, net_crs=net_crs, reference_crs=gis_data_crs)
    ...
    cost_assumptions = {
        'objektname': {
            'Wohnbaufläche': 65535,                             # Residential area - avoid
            'Industrie- und Gewerbefläche': 65535,              # Industrial area - avoid
            'Fläche besonderer funktionaler Prägung': 65535,    # Special functional area - avoid
            'Tagebau/Grube/Steinbruch': 65535,                  # Quarry/mining - avoid
            'Friedhof': 65535,                                  # Cemetery - avoid
            'Halde': 65535,                                     # Waste dump - avoid
            'Sumpf': 65535,                                     # Swamp - avoid
            'Flugverkehr': 65535,                               # Airport - avoid

            'Straßenverkehr': 178,                              # Road traffic
            'Sport-, Freizeit- und Erholungsfläche': 107,       # Sports/leisure area
            'Weg': 97,                                          # Path
            'Landwirtschaft': 107,                              # Agricultural land
            'Wald': 365,                                        # Forest
            'Fließgewässer': 155,                               # Flowing water
            'Gehölz': 365,                                      # Woodland
            'Fläche gemischter Nutzung': 107,                   # Mixed use area
            'Platz': 152,                                       # Square/plaza
            'Unland/Vegetationslose Fläche': 92,                # Wasteland/non-vegetated area
            'Stehendes Gewässer': 155,                          # Standing water
            'Bahnverkehr': 415,                                 # Railway
        }
    }
...
def get_cost(lines, line, ignore_category=65535):
    """
...
def find_routes_for_connection_lines(net, raster_path, result_path, sources, net_crs, gis_data_crs) -> gpd.GeoDataFrame:
    """
    ...
    all_targets = find_closest_connection_points(net, sources, nr_of_buses=20, net_crs=net_crs,
    ...
    for r in ['r0', 'r1', 'r2', 'r3']:
        if r == 'r3':
            ...
            # Each cluster contains 2 potential connection points to keep the graph size manageable
            for target_indices in [[133, 141],
                ...
                path_finder = pyorps.PathFinder(raster_path, source_coords=sources,
                                                target_coords=ts, neighborhood_str=r, ignore_max_cost=False)
            ...
            for path in paths:
                all_paths.add(path)
    all_paths_gdf = gpd.GeoDataFrame(all_paths.to_geodataframe_records(), crs=gis_data_crs)
    all_paths_gdf.to_file(result_path)
    return all_paths_gdf


def analyze_mv_oberrhein(result_path, net_crs, gis_data_crs) -> None:
    """
    ...
    trafo_stations = net.bus.loc[net.bus.index.isin(net.trafo.hv_bus) & (net.bus.vn_kv == 20)].index.to_list()

    # Change grid topology by moving a sectioning point (otherwise all neighborhoods lead to the same PCC)
    ...
    new_sgen = pp.create_sgen_from_cosphi(net, bus=from_bus, sn_mva=1.0, mode="underexcited", cos_phi=0.90)

    # Set some parameters to all static generators
    net.ext_grid.vm_pu = 1.04
    p_mw, q_mvar = pp.pq_from_cosphi(net.sgen.sn_mva, cosphi=0.90, qmode="underexcited", pmode="gen")
    net.sgen.p_mw = p_mw
    net.sgen.q_mvar = q_mvar

    # As this case study only serves to demonstrate the capabilities of PYORPS, we deliberately selected a very low
    # simultaneity factor for all PV systems connected to the medium voltage distribution grid.
    # This decision was made because high levels of simultaneous PV generation would exceed the technical limitations
    # of the existing grid infrastructure - even without any additional PV system!
    net.sgen.scaling = 0.7      # Low simultaneity for PV systems due to
...
def evaluate_connection_line(net, line_idx, from_bus, line_type, geom, neighborhood_lines, bus_geoseries,
                             original_sgen_p_mw, buses_memo):
    """
    ...
    net.sgen.p_mw = 0
    pp.runpp(net)
    overall_vdm_max = (all_vm - net.res_bus.loc[net.res_bus.index.isin(net.sgen.bus)].vm_pu.values).max()
    ...
    line_result_data['Euclidean distance [km]'] = neighborhood_lines.at[line_idx, 'euclidean_distance']
    line_result_data['Search space buffer [m]'] = neighborhood_lines.at[line_idx, 'search_space_buffer_m']
    line_result_data['Total runtime [s]'] = neighborhood_lines.at[line_idx, 'runtime_total']
    line_result_data['Time for path finding [s]'] = neighborhood_lines.at[line_idx, 'runtime_shortest_path']
    line_result_data['Time for path metrics [s]'] = neighborhood_lines.at[line_idx, 'runtime_path_metrics']
